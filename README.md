...
**An Apple Watch can use it too, through a Shortcut.** A real watchOS app needs a Mac and a paid developer account, so this goes the free route instead: `--phone` also serves `POST /say`, which takes text and returns Legion's reply as plain text. A three-action Shortcut on the iPhone (which syncs to the watch) does the rest — **Dictate Text**, then **Get Contents of URL** (`POST` to `http://<your PC's address>:8420/say`, request body *Form*, one field named `text` set to the dictated text), then **Speak Text** with the result — and turning on *Show on Apple Watch* in its settings puts it on the wrist. It uses the plain-HTTP port on purpose: the watch has no way to trust the self-signed certificate the phone page needs. Like the phone page, it only works on the home network, and anyone else on that network could send it text too.
